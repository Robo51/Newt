package robo51.newt.network.packet;
...
public abstract class AbstractMessageHandler<T extends IMessage> implements IMessageHandler <T, IMessage>
{
	...
	public IMessage onMessage(T message, MessageContext ctx) {
		if (ctx.side.isClient()) {
			return handleClientMessage(Newt.proxy.getPlayerEntity(ctx), message, ctx);
		} else {
			return handleServerMessage(Newt.proxy.getPlayerEntity(ctx), message, ctx);
		}
	}
}